...
import EditProductUserSideDialog from './EditProductUserSideDialog';
import DeleteProductUserSideDialog from './DeleteProductUserSideDialog';
import { ProductUserSideType } from '@/lib/schemas/ProductSchema';
import { useAuthContext } from '@/context/AuthContext';
import Spinner from '@/components/shared/Spinner';

export function ProductTableUserPanel({
  products,
}: {
  ...
  useEffect(() => {
    if (products) {
      ...
      setResolvedProducts(filterByUser);
      setIsLoading(false);
    }
        ...
        return (
          <>
            {product.addedByAdmin ? (
              <span className="inline-flex items-center px-2 py-2 text-sm border border-green-300 font-semibold text-green-800 bg-green-100 rounded-full">
                Added By Admin
              </span>
            ) : (
              <div className="flex items-center justify-start gap-2">
                <EditProductUserSideDialog defaultValues={product} />
                <DeleteProductUserSideDialog productId={product.id} />
              </div>
            )}
          </>
        );
