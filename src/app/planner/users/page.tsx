...
import { saveUserManagement, UserTeam, UserStatus } from '@/actions/planner/user-management';
...
function deriveTeam(u: PlannerRaw): UserTeam {
  if (u.team === 'XB' || u.team === 'TeQF' || u.team === 'none') return u.team as UserTeam;
  if (u.teamRole === 'xb_planner') return 'XB';
  if (u.teamRole === 'teqf_user' || u.teamRole === 'both') return 'TeQF';
  return 'none';
}

function permsFor(team: UserTeam): Permissions {
  return {
...
const PERM_ROWS: { key: string; label: string; tag: string; get: (p: Permissions) => boolean }[] = [
...
const TEAM_COLOR: Record<UserTeam, string> = {
  XB:   '#1d4ed8',
  TeQF: 'var(--tqf-bordeaux)',
  none: 'var(--tqf-muted)',
};
const TEAM_BG: Record<UserTeam, string> = {
  XB:   '#eff6ff',
      ...
      <div className="p-5 space-y-4">
        ...
        <div className="flex items-center gap-3">
          <div className="size-10 rounded-full flex items-center justify-center text-sm font-semibold flex-shrink-0"
            style={{ background: TEAM_BG[team], color: TEAM_COLOR[team], fontFamily: 'var(--font-body)' }}>
            {initial}
          </div>
        ...
        <div>
          ...
          <select value={team} onChange={e => setTeam(e.target.value as UserTeam)}
            style={{ ...selectBase, color: 'var(--tqf-dark)' }}>
            <option value="none">Nessuno</option>
            <option value="XB">XB</option>
            <option value="TeQF">TeQF</option>
          </select>
        </div>
              ...
              return (
                <div key={key} className="flex items-center gap-2.5">
                  ...
                  <span className="text-xs"
                    style={{ color: 'var(--tqf-muted)', fontFamily: 'var(--font-body)', opacity: 0.6 }}>
                    {tag}
                  </span>
                </div>
              );
        ...
        <div className="flex items-start gap-3 mb-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs mt-0.5" style={{ color: 'var(--tqf-muted)', fontFamily: 'var(--font-body)' }}>
              Assegna team e permessi automatici
            </p>
          </div>
