...
export async function compressFurnitureImage(file: File): Promise<File> {
  ...
  if (isHeic) {
    try {
      ...
      console.warn('[compressFurnitureImage] HEIC conversion fallita, si usa il file originale.', error);
    }
  }

  try {
    const timeout = new Promise<File>((resolve) =>
      setTimeout(() => resolve(normalizedFile), 12000)
    );
